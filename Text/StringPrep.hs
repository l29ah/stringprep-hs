...
mapToNothings = Set.fromAscList $
                  ['\x00AD', '\x034F', '\x1806', '\x180B', '\x180C','\x180D'
                  , '\x200B', '\x200C', '\x200D', '\x2060', '\xFE00', '\xFE01'
                  , '\xFE02','\xFE03', '\xFE04', '\xFE05', '\xFE06', '\xFE07'
                  , '\xFE08', '\xFE09', '\xFE0A', '\xFE0B', '\xFE0C', '\xFE0D'
                  , '\xFE0E', '\xFE0F', '\xFEFF']
...
textify :: (t, String) -> (t, Text)
textify (x,y) = (x, Text.pack y)

b2map :: Map.Map Char Text
b2map = Map.fromAscList . map textify $ [
        ('\x0041', "\x0061"),
        ('\x0042', "\x0062"),
        ('\x0043', "\x0063"),
        ('\x0044', "\x0064"),
